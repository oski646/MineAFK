...
def calculate_inventory_mouse_position(slot):
    ...
    if slot in first_row_indexes:
        return (
            config.slots["first_row_x"]
            + (first_row_indexes.index(slot) * config.slots["difference"]),
            config.slots["first_row_y"],
        )
    elif slot in second_row_indexes:
        return (
            config.slots["first_row_x"]
            + (second_row_indexes.index(slot) * config.slots["difference"]),
            config.slots["first_row_y"] + config.slots["difference"],
        )
    elif slot in third_row_indexes:
        return (
            config.slots["first_row_x"]
            + (third_row_indexes.index(slot) * config.slots["difference"]),
            config.slots["first_row_y"] + (2 * config.slots["difference"]),
        )
    elif slot in fourth_row_indexes:
        return (
            config.slots["first_row_x"]
            + (fourth_row_indexes.index(slot) * config.slots["difference"]),
            config.slots["first_row_y"] + (3 * config.slots["difference"]),
        )
...
def drop():
    ...
    for slot in drop_slots:
        drop_slot(
            calculate_inventory_mouse_position(slot)[0],
            calculate_inventory_mouse_position(slot)[1],
        )
...
def activity():
    ...
    while True:
        if activity_thread_stop == True:
            break
        if (
            activity_rounds == (config.activity_rounds_config - 1)
            or cobblex_rounds == (config.cobblex_rounds_config - 1)
            or drop_rounds == (config.drop_rounds_config - 1)
            or eat_rounds == (config.eat_rounds_config - 1)
        ):
...
with KeyboardManager.Listener(on_press=on_press, on_release=on_release) as listener:
    listener.join()
listener = KeyboardManager.Listener(on_press=on_press, on_release=on_release)
listener.start()
